...
class HCPlotAbandsOptions implements HCOption {
  ...
  /// (Highstock) Option for fill color between lines in Acceleration bands Indicator.
...
class HCPlotAoOptions implements HCOption {
  ...
  /// (Highstock) Color of the Awesome oscillator series bar that is greater than the previous one. Note that if a `color` is defined, the `color` takes precedence and the `greaterBarColor` is ignored.
  ...
  /// (Highstock) Color of the Awesome oscillator series bar that is lower than the previous one. Note that if a `color` is defined, the `color` takes precedence and the `lowerBarColor` is ignored.
...
class HCPlotApoOptions implements HCOption {
  ...
  /// (Highstock) Parameters used in calculation of Absolute Price Oscillator series points.
...
class HCPlotArcdiagramOptions implements HCOption {
  ...
  /// (Highcharts) The option to center links rather than position them one after another
  final bool? centeredLinks;
  ...
  /// (Highcharts, Highstock, Highmaps, Gantt) Options for the data labels appearing on top of the nodes and links. For arc diagram charts, data labels are visible for the nodes by default, but hidden for links. This is controlled by modifying the `nodeFormat`, and the `format` that applies to links and i
  final dynamic dataLabels;
  ...
  /// (Highcharts) Whether nodes with different values should have the same size. If set to true, all nodes are calculated based on the `nodePadding` and current `plotArea`. It is possible to override it using the `marker.radius` option.
  final bool? equalNodes;
  ...
  /// (Highcharts) Options for the point markers of arc diagram series. Properties like `fillColor`, `lineColor` and `lineWidth` define the visual appearance of the markers, while the `symbol` option defines their shape.
  final HCPointMarkerOptionsObject? marker;
  ...
  /// (Highcharts) Whether the series should be placed on the other side of the `plotArea`.
  final bool? reversed;
  ...
  Map<String, dynamic> toJson() {
    ...
    if (centeredLinks != null) {
      json['centeredLinks'] = centeredLinks;
    }
    if (className != null) {
      json['className'] = className;
    }
