...
class HCPlotFlagsOptions implements HCOption {
  ...
  /// (Highstock) Whether the flags are allowed to overlap sideways. If `false`, the flags are moved sideways using an algorithm that seeks to place every flag as close as possible to its original position.
  final bool? allowOverlapX;
  ...
  /// (Highcharts, Highstock, Gantt) The corner radius of the border surrounding each flag. For `squarepin` shaped flags only. A number signifies pixels. A percentage string, like for example 50%, signifies a relative size.
  final num? borderRadius;
  ...
  /// (Highstock) Fixed height of the flag's shape. By default, height is autocalculated according to the flag's title.
  final num? height;
  ...
  /// (Highstock) What type of legend symbol to render for this series. For flags series, the default is `flag-icon`, a pole with a pennant.
  ...
  /// (Highstock) In case the flag is placed on a series, on what point key to place it. Line and columns have one key, `y`. In range or OHLC-type series, however, the flag can optionally be placed on the `open`, `high`, `low` or `close` key.
  final String? onKey;

  /// (Highstock) Options for the _Series on point_ feature. Only `pie` and `sunburst` series are supported at this moment.
  final dynamic onPoint;

  /// (Highstock) The id of the series that the flags should be drawn on. If no id is given, the flags are drawn on the x axis.
  ...
  /// (Highstock) The shape of the marker. Can be one of "flag", "circlepin", "squarepin", or an image of the format `url(/path-to-image.jpg)`. Individual shapes can also be set for each point.
  final String? shape;
  ...
  /// (Highstock) When multiple flags in the same series fall on the same value, this number determines the vertical offset between them.
  ...
  /// (Highstock) The text to display on each flag. This can be defined on series level, or individually for each point. Defaults to `"A"`.
  final String? title;

  /// (Highstock) Specific tooltip options for flag series. Flag series tooltips are different from most other types in that a flag doesn't have a data value, so the tooltip rather displays the `text` option for each point.
  final HCSeriesTooltipOptionsObject? tooltip;

  /// (Highstock) Whether to use HTML to render the flag texts. Using HTML allows for advanced formatting, images and reliable bi-directional text rendering. Note that exported images won't respect the HTML, and that HTML won't respect Z-index settings.
  final bool? useHTML;

  /// (Highstock) Set the initial visibility of the series.
  final bool? visible;

  /// (Highstock) Fixed width of the flag's shape. By default, width is autocalculated according to the flag's title.
  final num? width;

  /// (Highstock) The y position of the top left corner of the flag relative to either the series (if onSeries is defined), or the x axis. Defaults to `-30`.
  final num? y;
  ...
  Map<String, dynamic> toJson() {
    ...
    if (allowOverlapX != null) {
      json['allowOverlapX'] = allowOverlapX;
    }
...
class HCPlotGanttAccessibilityPointOptions implements HCOption {
  ...
  /// (Gantt) Date format to use for points on datetime axes when describing them to screen reader users.
  final String? dateFormat;

  /// (Gantt) Whether or not to describe points with the value `null` to assistive technology, such as screen readers.
  final bool? describeNull;

  /// (Gantt) A format string to use instead of the default for point descriptions.
  final String? descriptionFormat;

  /// (Gantt) Decimals to use for the values in the point descriptions. Uses tooltip.valueDecimals if not defined.
  final num? valueDecimals;

  /// (Gantt) Format to use for describing the values of data points to assistive technology - including screen readers. The point context is available as `{point}`.
  final String? valueDescriptionFormat;

  /// (Gantt) Prefix to add to the values in the point descriptions. Uses tooltip.valuePrefix if not defined.
  final String? valuePrefix;

  /// (Gantt) Suffix to add to the values in the point descriptions. Uses tooltip.valueSuffix if not defined.
  final String? valueSuffix;
...
/// (Gantt) Options for the _Series on point_ feature. Only `pie` and `sunburst` series are supported at this moment.
class HCPlotGanttOnPointOptions implements HCOption {
  ...
  /// (Gantt) The `id` of the point that we connect the series to. Only points with a given `plotX` and `plotY` values and map points are valid.
  final String? id;

  /// (Gantt) Options allowing to set a position and an offset of the series in the _Series on point_ feature.
  final dynamic position;
...
class HCPlotGanttOnPointPositionOptions implements HCOption {
  ...
  /// (Gantt) Series center offset from the original x position. If defined, the connector line is drawn connecting original position with new position.
  final num? offsetX;

  /// (Gantt) Series center offset from the original y position. If defined, the connector line is drawn from original position to a new position.
  final num? offsetY;

  /// (Gantt) X position of the series center. By default, the series is displayed on the point that it is connected to.
  final num? x;

  /// (Gantt) Y position of the series center. By default, the series is displayed on the point that it is connected to.
  final num? y;
...
class HCPlotGanttOptions implements HCOption {
  ...
  /// (Gantt) Allow this series' points to be selected by clicking on the graphic (columns, point markers, pie slices, map areas etc).
  final bool? allowPointSelect;

  /// (Gantt) Enable or disable the initial animation when a series is displayed. The animation can also be set as a configuration object. Please note that this option only applies to the initial animation of the series itself. For other animations, see chart.animation and the animation parameter under th
  final dynamic animation;

  /// (Gantt) For some series, there is a limit that shuts down animation by default when the total number of points in the chart is too high. For example, for a column chart and its derivatives, animation does not run if there is more than 250 points totally. To disable this cap, set `animationLimit` to
  final num? animationLimit;
  ...
  /// (Gantt) An additional class name to apply to the series' graphical elements. This option does not replace default class names of the graphical element. Changes to the series' color will also be reflected in a chart's legend and tooltip.
  final String? className;

  /// (Gantt) Disable this option to allow series rendering in the whole plotting area.
  final bool? clip;

  /// (Gantt) The main color of the series. In line type series it applies to the line and the point markers unless otherwise specified. In bar type series it applies to the bars unless a color is specified per point. The default value is pulled from the `options.colors` array.
  final dynamic color;

  /// (Highcharts, Highstock, Gantt) In an X-range series, this option makes all points of the same Y-axis category the same color.
  final bool? colorByPoint;

  /// (Gantt) Styled mode only. A specific color index to use for the series, so its graphic representations are given the class name `highcharts-color-{n}`.
  final num? colorIndex;
  ...
  /// (Gantt) You can set the cursor to "pointer" if you have click events attached to the series, to signal to the user that the points and lines can be clicked.
  final String? cursor;
  ...
  /// (Gantt) Name of the dash style to use for the graph, or for some series types the outline of each shape.
  final String? dashStyle;

  /// (Highcharts, Highstock, Highmaps, Gantt) Options for the series data labels, appearing next to each data point.
  final dynamic dataLabels;

  /// (Gantt) The mapping between the data table and the series data points. This is used in conjunction with the `dataTable` option (on chart or series level) to map columns from the data table to the properties of the data points. The keys of the `dataMapping` object correspond to the properties of the
  final HCDataMappingOptionsObject? dataMapping;

  /// (Gantt) Deprecated. Use plotOptions.series.accessibility.description instead.
  final String? description;

  /// (Gantt) The draggable-points module allows points to be moved around or modified in the chart. In addition to the options mentioned under the `dragDrop` API structure, the module fires three events, point.dragStart, point.drag and point.drop.
  final HCSeriesDragDropOptionsObject? dragDrop;

  /// (Gantt) Enable or disable the mouse tracking for a specific series. This includes point tooltips and click events on graphs and points. For large datasets it improves performance.
  final bool? enableMouseTracking;

  /// (Gantt) General event handlers for the series items. These event hooks can also be attached to the series at run time using the `Highcharts.addEvent` function.
  final HCSeriesEventsOptionsObject? events;
  ...
  /// (Gantt) When set to `false` will prevent the series data from being included in any form of data export.
  final bool? includeInDataExport;

  /// (Gantt) An array specifying which option maps to which key in the data point array. This makes it convenient to work with unstructured data arrays from different sources.
  final List<String>? keys;

  /// (Highcharts, Highstock, Gantt) Series labels are placed as close to the series as possible in a natural way, seeking to avoid other series. The goal of this feature is to make the chart more easily readable, like if a human designer placed the labels in the optimal position.
  final HCSeriesLabelOptionsObject? label;

  /// (Gantt) What type of legend symbol to render for this series. Can be one of `areaMarker`, `lineMarker` or `rectangle`.
  final String? legendSymbol;
  ...
  /// (Highcharts, Highstock, Gantt) A partial fill for each point, typically used to visualize how much of a task is performed.
  final HCPlotGanttPartialFillOptions? partialFill;

  /// (Gantt) Properties for each single point.
  final HCPlotSeriesPointOptions? point;

  /// (Gantt) Deprecated. Use series.accessibility.point.descriptionFormat instead.
  final dynamic pointDescriptionFormat;

  /// (Gantt) Deprecated. Use series.accessibility.point.descriptionFormatter instead.
  final dynamic pointDescriptionFormatter;
  ...
  /// (Gantt) Whether to select the series initially. If `showCheckbox` is true, the checkbox next to the series name in the legend will be checked for a selected series.
  final bool? selected;

  /// (Gantt) Whether to apply a drop shadow to the graph line. Since 2.3 the shadow can be an object configuration containing `color`, `offsetX`, `offsetY`, `opacity` and `width`.
  final dynamic shadow;

  /// (Gantt) If true, a checkbox is displayed next to the legend item to allow selecting the series. The state of the checkbox is determined by the `selected` option.
  final bool? showCheckbox;

  /// (Gantt) Whether to display this particular series or series type in the legend. Standalone series are shown in legend by default, and linked series are not. Since v7.2.0 it is possible to show series that use colorAxis by setting this option to `true`.
  final bool? showInLegend;
  ...
  /// (Gantt) Sticky tracking of mouse events. When true, the `mouseOut` event on a series isn't triggered until the mouse moves over another series, or out of the plot area. When false, the `mouseOut` event on a series is triggered when the mouse leaves the area around the series' graph or markers. This
  final bool? stickyTracking;

  /// (Gantt) A configuration object for the tooltip rendering of each single series. Properties are inherited from tooltip, but only the following properties can be defined on a series level.
  final HCSeriesTooltipOptionsObject? tooltip;
  ...
  /// (Gantt) Whether to zoom non-cartesian series. If `chart.zooming` is set, the option allows to disable zooming on an individual non-cartesian series. By default zooming is enabled for all series.
  final bool? zoomEnabled;
...
/// (Gantt) Options for the tooltip header when tooltip.split is enabled. The header is the box containing the X value in a split tooltip.
class HCPlotGanttTooltipHeaderOptions implements HCOption {
  ...
  /// (Gantt) Background color for the tooltip header when tooltip.split is enabled.
  final dynamic backgroundColor;

  /// (Gantt) Border color for the tooltip header when tooltip.split is enabled.
  final dynamic borderColor;

  /// (Gantt) The width of the border for the tooltip header when tooltip.split is enabled.
  final num? borderWidth;

  /// (Gantt) Distance between the plot area and the header (except the chevron) in a split tooltip, in pixels. The default value makes the header text align with the axis labels.
  final num? distance;

  /// (Gantt) The name of a symbol to use for the border around the tooltip header. Applies only when tooltip.split is enabled.
  final String? shape;

  /// (Gantt) CSS styles for the tooltip header. The default is `{ fontSize: '1em' }`, ensuring that the header text is the same size as the axis labels.
  final dynamic style;
...
/// (Gantt) Positioning options for fixed tooltip, taking effect only when tooltip.fixed is `true`.
class HCPlotGanttTooltipPositionOptions implements HCOption {
  ...
  /// (Gantt) X pixel offset from the given position. Can be used to shy away from axis lines, grid lines etc to avoid the tooltip overlapping other elements.
  final num? x;

  /// (Gantt) Y pixel offset from the given position. Can be used to shy away from axis lines, grid lines etc to avoid the tooltip overlapping other elements.
  final num? y;
...
class HCPlotGaugeDialOptions implements HCOption {
  ...
  /// (Highcharts) The length of the dial's base part, relative to the total radius or length of the dial. Accepts a pixel value if given as a number, or a percentage value if given as a percentage string. If the base length is greater than 0, the dial's base will have an even width, before it narrows in
  final dynamic baseLength;

  /// (Highcharts) The width of the base of the gauge dial. The base is the part closest to the pivot, defined by baseLength. Accepts a pixel value if given as a number, or a percentage value if given as a percentage string.
  final dynamic baseWidth;

  /// (Highcharts) The border color or stroke of the gauge's dial. By default, the borderWidth is 0, so this must be set in addition to a custom border color.
  final dynamic borderColor;
  ...
  /// (Highcharts) The radius or length of the dial, relative to the radius of the gauge itself. Accepts a pixel value if given as a number, or a percentage value if given as a percentage string.
  final dynamic radius;

  /// (Highcharts) The length of the dial's rear end, the part that extends out on the other side of the pivot. Accepts a pixel value if given as a number, or a percentage value of the dial's length if given as a percentage string.
  final dynamic rearLength;

  /// (Highcharts) The width of the top of the dial, closest to the perimeter. The pivot narrows in from the base to the top. Accepts a pixel value if given as a number, or a percentage of the dial radius if given as a percentage string.
...
class HCPlotGaugeOptions implements HCOption {
  ...
  /// (Highcharts) Allow the dial to overshoot the end of the perimeter axis by this many degrees. Say if the gauge axis goes from 0 to 60, a value of 100, or 1000, will show 5 degrees beyond the end of the axis when this option is set to 5.
  final num? overshoot;
  ...
  /// (Highcharts) When this option is `true`, the dial will wrap around the axes. For instance, in a full-range gauge going from 0 to 360, a value of 400 will point to 40. When `wrap` is `false`, the dial stops at 360.
  final bool? wrap;
...
class HCPlotGaugePivotOptions implements HCOption {
  ...
  /// (Highcharts) The radius of the pivot, the center point of the gauge. Accepts a pixel value if given as a number, or a percentage of the full gauge radius if given as a percentage string.
  final dynamic radius;
...
/// (Highmaps) Make the geoheatmap render its data points as an interpolated image. It can be used to show a Temperature Map-like charts.
class HCPlotGeoheatmapInterpolationOptions implements HCOption {
  ...
  /// (Highmaps) Represents how much blur should be added to the interpolated image. Works best in the range of 0-1, all higher values would need a lot more performance of the machine to calculate more detailed interpolation.
  final num? blur;

  /// (Highmaps) Enable or disable the interpolation of the geoheatmap series.
  final bool? enabled;
...
class HCPlotGeoheatmapOptions implements HCOption {
  ...
  /// (Highmaps) The main color of the series. In heat maps this color is rarely used, as we mostly use the color to denote the value of each point. Unless options are set in the colorAxis, the default value is pulled from the options.colors array.
  final dynamic color;
  ...
  /// (Highmaps) The column size - how many longitude units each column in the geoheatmap should span.
  final num? colsize;
  ...
  /// (Highmaps) The rowsize size - how many latitude units each row in the geoheatmap should span.
  final num? rowsize;
...
class HCPlotHeatmapOptions implements HCOption {
  ...
  /// (Highcharts, Highmaps) The column size - how many X axis units each column in the heatmap should span.
  final num? colsize;
  ...
  /// (Highcharts, Highmaps) The row size - how many Y axis units each heatmap row should span.
  final num? rowsize;
