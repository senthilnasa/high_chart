...
class HighChartsState extends State<HighCharts> {
  ...
  Future<void> initPlatformState() async {
    try {
      ...
      // `sendToFlutter` JS helper (window.chrome.webview.postMessage):
      // exported-file payloads are handled internally (saved to disk),
      // everything else goes to `onEvent`.
      _webMessageSubscription = _controller.webMessage.listen(
        (message) {
          if (message is Map && message['__highChartsExport'] == true) {
            _handleExport(message);
            return;
          }
  ...
  // Where exported files are saved: the real Downloads directory on
  // Windows, falling back to the app's documents directory if that's ever
  // unavailable.
  Future<Directory> _exportSaveDirectory() async {
    try {
      ...
      // No downloads directory available; fall through to the fallback.
    }
  ...
  void _loadData() {
    if (mounted) {
      ...
      // Inject the chart data into the WebView. `executeScript` runs
      // immediately against the already-loaded document, unlike
      // `addScriptToExecuteOnDocumentCreated` which only applies to future
      // navigations and would never fire for this page. Errors (e.g. a
